...
import { Circle, Play, Plus, Settings, User } from "lucide-react";
...
export function MainMenu({ actions }: MainMenuProps) {
  return (
    <>
      ...
      <a className="flex flex-col items-center justify-center text-on-primary bg-primary rounded-none w-full h-full border-t-2 border-primary" href="#">
      <Circle aria-hidden={true} focusable="false" />
      <span className="text-label-sm font-label-sm mt-1">Battle</span>
      </a>
      <a className="flex flex-col items-center justify-center text-on-surface-variant dark:text-on-surface-variant w-full h-full hover:bg-surface-container-high transition-colors duration-200" href="#">
      <Circle aria-hidden={true} focusable="false" />
